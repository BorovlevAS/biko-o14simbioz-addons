...
class AccountPaymentP24(models.Model):
    ...
    def _compute_hide_p24b(self):
        for rec in self:
            rec.hide_p24 = not (rec.journal_id and rec.journal_id.bank_statements_source == 'p24_import')

    hide_p24 = fields.Boolean(
        compute='_compute_hide_p24b',
    )
    is_exported = fields.Boolean(
        string='Exported to P24'
    )
